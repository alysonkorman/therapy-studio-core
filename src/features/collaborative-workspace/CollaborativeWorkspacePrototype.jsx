...
import { calculateInitialWorkspaceObjectSize } from "./workspaceDocument";
  ...
  function handleSelectedAction(action) {
    ...
    if (action === "delete") {
      ...
      changeDocument({
        type: "object/update",
        objectId: selectedId,
        changes: { rotation: selectedObject.rotation + 15 },
      });
      ...
      const shortestSide = Math.min(selectedObject.width, selectedObject.height);
      const minimumScale = 72 / shortestSide;
      const scale = Math.max(minimumScale, (longestSide + amount) / longestSide);
      changeDocument({
        type: "object/update",
        objectId: selectedId,
        changes: {
          width: selectedObject.width * scale,
          height: selectedObject.height * scale,
        },
      });
    } else {
      changeDocument({
